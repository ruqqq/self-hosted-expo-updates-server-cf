...
import type { Env, ExpoRequestContext, ExpoManifest, ExpoAsset, ManifestExtensions } from '../types'
import { uploads, clients, apps } from '../db/schema'
import { parseExpoRequest, createMultipartResponse, signManifest, hashAsset, md5Hash } from '../services/manifest'
...
api.get('/manifest', async (c) => {
  ...
  const { project, platform, runtimeVersion, releaseChannel, expectSignature, clientId } = params
  ...
  return createMultipartResponse(manifest, extensions, signature, params.protocolVersion)
})
...
async function trackClient(db: ReturnType<typeof drizzle>, params: ExpoRequestContext) {
  const now = new Date()
